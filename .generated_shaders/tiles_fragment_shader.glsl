...
vec4 tiles_shadertoy_color(vec2 frag_coord, vec2 resolution, float time);

vec4 tiles_shadertoy_color(vec2 frag_coord, vec2 resolution, float time) {
     float aspect_ratio = resolution.y / resolution.x;
     vec2 uv = frag_coord / resolution.x;
...
void main() {
     out_color = tiles_shadertoy_color(vec2(gl_FragCoord.x, gl_FragCoord.y), u_resolution, u_time);
}
